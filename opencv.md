...
虽然Harris和Shi-Tomas具有 `旋转不变性` ，但是不具有 `尺度不变性` 
...
SIFT算法步骤：1、尺度空间极值检测；2、关键点定位；3、关键点方向确定；4、关键点描述；
...
Fast是一种角点检测算法，其原理是取图像中检测点，以该点为圆心的周围邻域内像素点判断所检测的点是否为角点，通俗的讲就是 **若一个像素周围有一定数量的像素与该点像素值不同，则认为其为角点**
